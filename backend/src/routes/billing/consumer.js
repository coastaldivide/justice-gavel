...
router.post('/consumer/subscribe', billingLimiter, authRequired, async (req, res) => {
  ...
  const consumerTiers = ['advisor', 'legal_pro', 'legal_radar'];
  if (!consumerTiers.includes(tier)) {
    return err400(res, 'Invalid consumer tier. Options: starter, pro, legal_radar');
  }
...
router.get('/consumer/subscription', authRequired, async (req, res) => {
  try {
    ...
    const consumerTiers = ['advisor', 'legal_pro', 'legal_radar'].map(k => ({ key: k, ...TIERS[k] }));
