...
describe('BLK_E. firms(5) + recap(5) + caldav(5) + acquisition(4) Double', () => {
  test('E-01: firms invite + accept + DELETE + PATCH member + audit', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/firms.js','utf8');
  ...
  test('E-03: caldav ical-token + push/matter + ical + DELETE events + push/entryId', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/integrations/caldav.js','utf8');
...
describe('BLK_F. All Remaining 46 Routes — Single Pass', () => {
  ...
  test('F-03: integrations-index + outbound + consultations + dms + providers', async () => {
    ...
    const dm=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/integrations/dms.js','utf8');
  ...
  test('F-04: golden_gavel + lessons + auth + billing suite + attorneys', async () => {
    ...
    const au=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/auth.js','utf8');
  ...
  test('F-06: translate + recovery + legaldata + sso + motions + billing/subs + pi_leads', async () => {
    const fs=await import('fs');
    const tr=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/translate.js','utf8');
    const ra=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/recovery_agents.js','utf8');
    const ld=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/legaldata.js','utf8');
    const ss=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/sso.js','utf8');
    const me=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/motions/export.js','utf8');
    const bs=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/subscriptions.js','utf8');
    const pl=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/pi_leads.js','utf8');
  ...
  test('F-07: resources + webpush + attorney/templates + verification', async () => {
    const fs=await import('fs');
    const re=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/resources.js','utf8');
    const wp=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/webpush.js','utf8');
    const at=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/attorney/templates.js','utf8');
