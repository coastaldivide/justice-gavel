...
describe('JOURNEY. All 12 User Journeys', () => {
  ...
  test('JOURNEY-11: Attorney Dashboard — profile + cases + bar validation', async () => {
    const fs=await import('fs'); const path=await import('path');
    const atty=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/AttorneyDashboardScreen.tsx','utf8');
...
describe('MASS. 2M Influx', () => {
  test('MASS-01: 2M escalation + 2M encrypt zero errors', () => {
    ...
    expect(e).toBe(0);
    let e2=0;
    for(let i=0;i<5000;i++) if(decrypt(encrypt('v178_'+i))!==('v178_'+i))e2++;
    expect(e2).toBe(0);
  });
});
