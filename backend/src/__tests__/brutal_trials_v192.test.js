...
describe('JOURNEY. 10 Critical User Journeys', () => {
  test('JOURNEY-01: Just Arrested → SOS + HelpNow provider search', async () => {
    ...
    const em = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/EmergencyScreen.tsx','utf8');
  ...
  test('JOURNEY-02: Login → token storage → app routing → biometric gate', async () => {
    const fs = await import('fs');
    const login = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/LoginScreen.tsx','utf8');
  ...
  test('JOURNEY-03: Find Lawyer → filter/search → profile → contact/save', async () => {
    const fs = await import('fs');
    const law  = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/LawyersScreen.tsx','utf8');
    const prof = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/LawyerProfileScreen.tsx','utf8');
  ...
  test('JOURNEY-05: Case management → load → notes → timeline → messages', async () => {
    const fs = await import('fs');
    const c = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/CaseScreen.tsx','utf8');
  ...
  test('JOURNEY-07: Bail Search → provider list → contact bondsman', async () => {
    const fs = await import('fs');
    const bail = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/BailSearchScreen.tsx','utf8');
  ...
  test('JOURNEY-10: Family Connect → search arrest → set alert → bail info', async () => {
    const fs = await import('fs');
    const fam = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/FamilyConnectScreen.tsx','utf8');
...
describe('MASS. 2M Influx', () => {
  test('MASS-01: 2M escalation + 2M encrypt zero errors', () => {
    ...
    expect(e).toBe(0);
    let e2=0;
    for(let i=0;i<5000;i++) if(decrypt(encrypt('v192_'+i))!==('v192_'+i)) e2++;
    expect(e2).toBe(0);
  });
});
