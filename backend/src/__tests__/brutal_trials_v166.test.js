...
describe('SCREEN. All 75 Screens — Zero Defects', () => {
  ...
  test('S-06: 0 components unused', async () => {
    ...
    let allSrc=fs.readFileSync('/tmp/JG_fresh/frontend/src/navigation/AppNavigator.tsx','utf8');
...
describe('NAV. All 76 Screens Reachable', () => {
  ...
  test('NAV-03: HelpNow→BailCalculator, HelpNow→CourtLocator correct names', async () => {
    const fs=await import('fs');
    const hns=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/HelpNowScreen.tsx','utf8');
  ...
  test('NAV-05: AgeGate navigates to Onboarding on success', async () => {
    const fs=await import('fs');
    const src=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/AgeGateScreen.tsx','utf8');
...
describe('BE. Backend Integrity', () => {
  ...
  test('BE-04: motions/generate.js route exists + mounted', async () => {
    ...
    const idx=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/motions/index.js','utf8');
...
describe('FLOW. Core User Journeys', () => {
  ...
  test('FLOW-06: Bail chain — Search→Calculator→Bondsman', async () => {
    const fs=await import('fs');
    const bs=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/BailSearchScreen.tsx','utf8');
    const bc=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/BailCalculatorScreen.tsx','utf8');
  ...
  test('FLOW-07: Know Your Rights chain', async () => {
    const fs=await import('fs');
    const ls=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/LessonsScreen.tsx','utf8');
  ...
  test('FLOW-09: Monitoring chain — Arrest Monitor → Family Connect → CheckIn', async () => {
    const fs=await import('fs');
    const am=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/ArrestMonitorScreen.tsx','utf8');
    const fc=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/FamilyConnectScreen.tsx','utf8');
    const ci=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/CheckInScreen.tsx','utf8');
  ...
  test('FLOW-10: Immigration chain — IceDetention→Immigration→HagueContact', async () => {
    const fs=await import('fs');
    const ice=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/IceDetentionScreen.tsx','utf8');
    const imm=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/ImmigrationConsequencesScreen.tsx','utf8');
    const hag=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/HagueContactScreen.tsx','utf8');
...
describe('MASS. 2M Influx — Zero Errors', () => {
  test('MASS-01: 2M escalation + 2M encrypt', () => {
    ...
    expect(e).toBe(0);
    let e2=0;
    for(let i=0;i<5000;i++) if(decrypt(encrypt(`v166_${i}`))!==`v166_${i}`) e2++;
    expect(e2).toBe(0);
  });
