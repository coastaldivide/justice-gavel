...
describe('DB_TABLES. 5 Missing Tables Now in Schema', () => {
  ...
  test('DB-03: migration 043 SQL file exists', async () => {
    const fs=await import('fs');
    expect(fs.existsSync('/tmp/JG_fresh/backend/src/migrations/043_missing_tables.sql')).toBe(true);
    const sql=fs.readFileSync('/tmp/JG_fresh/backend/src/migrations/043_missing_tables.sql','utf8');
...
describe('FALLBACK. setState Null Fallbacks Added', () => {
  ...
  test('FALLBACK-04: ConsumerSub + Insurance + RightsCard + CheckIn null fallbacks', async () => {
    const fs=await import('fs');
    const con=fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/ConsumerSubscriptionScreen.tsx','utf8');
...
describe('FINAL_159. Zero-Defect Quality Gate', () => {
  ...
  test('FINAL-03: 1M escalation + 1M encrypt', () => {
    ...
    expect(e).toBe(0);
    let e2=0;
    for(let i=0;i<10000;i++) if(decrypt(encrypt(`v159_${i}`))!==`v159_${i}`) e2++;
    expect(e2).toBe(0);
  });
});