...
describe('REF_A. referrals + arrests + messages + sso + admin → ≥15', () => {
  ...
  test('REF_A-03: messages read + stream + attachment + unread/count', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/messages.js','utf8');
  ...
  test('REF_A-04: sso test/:firmId + DELETE/GET/POST /config/:firmId', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/sso.js','utf8');
  ...
  test('REF_A-05: admin health-scan/history + log + health-scan/latest + run', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/admin.js','utf8');
...
describe('AUD_A. audit + consultations + dms + webhooks/outbound → ≥15', () => {
  test('AUD_A-01: audit contract/:id + user/:id + matter/:id', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/audit.js','utf8');
  ...
  test('AUD_A-02: consultations cancel + callback-request + slots', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/consultations.js','utf8');
  ...
  test('AUD_A-03: integrations/dms workspaces + POST workspaces + map', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/integrations/dms.js','utf8');
  ...
  test('AUD_A-04: webhooks/outbound deliveries + retry + test', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/webhooks/outbound.js','utf8');
...
describe('MISC_D. providers + research + lessons + docket + auth → ≥15', () => {
  test('MISC_D-01: providers coverage + nearest-city', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/providers.js','utf8');
  ...
  test('MISC_D-02: research DELETE/GET /session/:id', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/research.js','utf8');
  ...
  test('MISC_D-03: lessons GET/GET /progress', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/lessons.js','utf8');
  ...
  test('MISC_D-04: docket GET /upcoming + POST /calculate', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/docket.js','utf8');
  ...
  test('MISC_D-05: auth POST /update-profile + GET /tos-status', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/auth.js','utf8');
...
describe('MISC_E. motions + billing + attorney + single-routes → ≥15', () => {
  test('MISC_E-01: motions/export refine + preview; motions/history DELETE/GET', async () => {
    const fs=await import('fs');
    const me=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/motions/export.js','utf8');
    const mh=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/motions/history.js','utf8');
  ...
  test('MISC_E-02: billing/consumer admin/stats + subscribe; billing/connections', async () => {
    const fs=await import('fs');
    const bc=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/consumer.js','utf8');
  ...
  test('MISC_E-03: attorney/cle transcript + complete; attorney/cases assign + join', async () => {
    const fs=await import('fs');
    const cle=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/attorney/cle.js','utf8');
    const cas=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/attorney/cases.js','utf8');
  ...
  test('MISC_E-04: resources + webpush + translate + recovery_agents + legaldata', async () => {
    const fs=await import('fs');
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/resources.js','utf8')).toContain("router.get('/categories'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/webpush.js','utf8')).toContain("router.get('/key'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/translate.js','utf8')).toContain("router.get('/session/:code/messages'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/recovery_agents.js','utf8')).toContain("router.get('/laws/:state'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/legaldata.js','utf8')).toContain('/:type');
  });
  test('MISC_E-05: expungement/attorneys + motions/review + billing subs + pi_leads + verif + templates', async () => {
    const fs=await import('fs');
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/expungement/attorneys.js','utf8')).toContain("router.get('/attorneys'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/motions/review.js','utf8')).toContain("router.patch('/:id/status'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/subscriptions.js','utf8')).toContain("router.post('/refund'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/pi_leads.js','utf8')).toContain("router.post('/pi-lead/accept/:id'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/attorney/verification.js','utf8')).toContain("router.post('/approve-verification'");
    expect(fs.readFileSync('/tmp/JG_fresh/backend/src/routes/attorney/templates.js','utf8')).toContain("router.patch('/templates/:id/approve'");
  });
  test('MISC_E-06: golden_gavel evaluate + hall/opt-in + eligibility', async () => {
    const fs=await import('fs');
    const s=fs.readFileSync('/tmp/JG_fresh/backend/src/routes/golden_gavel.js','utf8');
