...
describe('CONTRACT. API Response Fields Match FE Expectations', () => {
  test('CONTRACT-01: messages.js SELECT cm.* covers .body field FE uses', async () => {
    ...
    const msg = fs.readFileSync('/tmp/JG_fresh/backend/src/routes/messages.js','utf8');
  ...
  test('CONTRACT-02: /family/contacts returns u.phone for emergency dialing', async () => {
    const fs = await import('fs');
    const cases = fs.readFileSync('/tmp/JG_fresh/backend/src/routes/cases.js','utf8');
...
describe('SESSION. Complete User Sessions', () => {
  test('SESSION-01: New user onboarding uses auth context (not navigate)', async () => {
    const fs = await import('fs');
    const onb = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/OnboardingScreen.tsx','utf8');
  ...
  test('SESSION-05: BailCalculator shows bondCost result in render', async () => {
    const fs = await import('fs');
    const bail = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/BailCalculatorScreen.tsx','utf8');
...
describe('PAY. Payment System Verified', () => {
  test('PAY-01: subscription flow complete', async () => {
    const fs = await import('fs');
    const sub = fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/subscriptions.js','utf8');
    ...
    // FE
    const feSub = fs.readFileSync('/tmp/JG_fresh/frontend/src/screens/SubscriptionScreen.tsx','utf8');
  ...
  test('PAY-02: QuickConnect charges + returns contacts', async () => {
    const fs = await import('fs');
    const qc = fs.readFileSync('/tmp/JG_fresh/backend/src/routes/billing/connections.js','utf8');
...
describe('MASS. 2M Influx', () => {
  test('MASS-01: 2M escalation + 2M encrypt zero errors', () => {
    ...
    expect(e).toBe(0);
    let e2=0;
    for(let i=0;i<5000;i++) if(decrypt(encrypt(`v174_${i}`))!==`v174_${i}`) e2++;
    expect(e2).toBe(0);
  });
});
