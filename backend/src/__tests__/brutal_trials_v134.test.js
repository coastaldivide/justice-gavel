...
describe('CTRC2. contracts/ — Full Document Lifecycle', () => {
  ...
  test('CTRC2-04: _contract_types.js + _helpers.js — shared contract utilities', async () => {
    const fs = await import('fs');
    const ct = fs.readFileSync('/tmp/JG_fresh/backend/src/routes/contracts/_contract_types.js','utf8');
    const ch = fs.readFileSync('/tmp/JG_fresh/backend/src/routes/contracts/_helpers.js','utf8');
